...
const UnknownIcon =
  require("../../../src/assets/images/icons/noImage.jpeg").default;
// ** Styles

const WalletView = ({ open, onEdit, handleModal, modal }) => {
  const intl = useIntl();
  const [logo, setLogo] = useState(
    modal.refImage ? modal.refImage : UnknownIcon
  );
  const [secondaryImage, setSecondaryImage] = useState(
    modal.secondary_image ? modal.secondary_image : UnknownIcon
  );
  return (
    <Modal
      isOpen={open}
    ...
    >
      ...
      <ModalBody className="px-sm-5 mx-50 pb-5 modal-body">
        <h1 className="text-center pb-2">{modal.identifier}</h1>
        <Row className=" py-1">
          <Col lg="6" sm="6">
            <FormGroup>
