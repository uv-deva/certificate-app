...
import { searchNodeTypes } from '@src/redux/actions/routes'
...
const NodeModal = ({ open, handleModal, modal }) => {
    const intl = useIntl()
    const dispatch = useDispatch()
    const typesSuggestion = useSelector(selectData('nodeTypes'))
    const [types, setTypes] = useState(modal && modal.refType[0] ? JSON.stringify(modal.refType[0]) : null)

    let loading = useSelector(selectTableAddLoading('nodes')) //get loading status of adding new data action
    let isDone = useSelector(selectTableAddIsDone('nodes'))  //get isDone status of adding new data action completed or not
    let { validations, errorText } = useSelector(selectAddErrors('nodes')) //get server field validations and non field errors for adding action
    if (modal && modal.id) {
      //if modal in initiated, it means that the form is an edit/update form
      loading = useSelector(selectTableUpdateLoading('nodes', modal.id)) //get loading status of updating data action
      isDone = useSelector(selectTableUpdateIsDone('nodes', modal.id)) //get isDone status of updating data action completed or not
      validations = useSelector(selectUpdateErrors('nodes', modal.id)).validations //get server field validations for updating action
      errorText = useSelector(selectUpdateErrors('nodes', modal.id)).errorText //get server non field errors for updating action
    }
   ...
   //handle form submit data, based on whether add or update form called depending action
    const onSubmit = data => {
      ...
      if (!modal) dispatch(ontTableRowAdd('nodes', resp)); else dispatch(ontTableRowUpdate('nodes', modal.id, resp))
    }
    ...
    >
      <ModalHeader className='mb-3' toggle={handleModal} close={CloseBtn} tag='div'>
        <h5 className='modal-title'>{!modal ? intl.formatMessage({id:"Add New Node"}) : intl.formatMessage({id:"Update Node"})}</h5>
      </ModalHeader>
      <ModalBody className='flex-grow-1'>
      <Form onSubmit={handleSubmit(onSubmit)}>
        <FormGroup>
          <Label for='name'>{intl.formatMessage({id:"Name"})}<span className="text-danger">*</span> <Info id="nodeName_tooltip" color="#45866E" size={14}/></Label>
          <InputGroup>
            ...
            />
            <UncontrolledTooltip placement='right' target='nodeName_tooltip'>
            Enter the full name
            </UncontrolledTooltip>
          </InputGroup>
        ...
        <FormGroup>
          ...
          <InputGroup>
            ...
            <Input 
                ...
                placeholder={intl.formatMessage({id:"e.g. https://www.xxx.com"})} 
            />
